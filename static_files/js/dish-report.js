        ...
        if (selected == "All Dishes"){
            ...
            for (var i = 0; i < chart1Data.length; i++){
                ...
                var s = $(`#dataTable${i}`).DataTable({
                    ajax: {
                        url: `/api/dish-report/${chart1Data[i].id}/${start_date}/${end_date}`,
                        type: "GET"
                    },
                    dataSrc: 'data',
                    columns: [
                ...
                s.on('xhr', function (e, settings, json, xhr){
                    if (json.data.length > 0)
                        rows.push([json.data[0].Dish, json.data.length]);

                    if(i >= chart1Data.length - 1){

                        heading = "The number of ordered dish.";

                        var data = {
                            header: ["Dish Name", "Orders Served"],
                            rows: rows
                        }
                    
                        chart.xAxis().title("Dishes");
                        chart.yAxis().title("No. of Orders");
                        chart.data(data);
            ...
            heading = "The number of ordered: " + selected;
            $("#dataTable").show();


            var s = $("#dataTable").DataTable({
                ajax: {
                    url: `/api/dish-report/${id}/${start_date}/${end_date}`,
                    type: "GET"
                },
                dataSrc: 'data',
                columns: [
            ...
            s.on('xhr', function (e, settings, json){
                ...
                var data = {
                    header: ["Dish Name", "Times Ordered"],
                    rows: rows
                }
            
                chart.xAxis().title("Dish");
                chart.yAxis().title("Times Ordered");
                chart.data(data);
