$(document).ready(()=>{
    ...
    $("#btn").on('click', function(e){
        ...
        // assign default start and end dates.
        ...
        if (selected == "All Waiters"){
            ...
            for (var i = 0; i < chart1Data.length; i++){
                console.log(`/api/waiter-report/${chart1Data[i].id}/${start_date}/${end_date}`);
                var s = $(`#dataTable${i}`).DataTable({
                    ajax: {
                        url: `/api/waiter-report/${chart1Data[i].id}/${start_date}/${end_date}`,
                        type: "GET"
                    },
                    dataSrc: 'data',
                    columns: [
                ...
                s.on('xhr', function (e, settings, json){
                    rows.push([json.data[0].Waiter, json.data.length]);
                    if(i >= chart1Data.length - 1){

                        heading = "The number of orders served by each waiter";

                        var data = {
                            header: ["Waiter Name", "Orders Served"],
                            rows: rows
                        }
                    
                        chart.xAxis().title("Waiters");
                        chart.yAxis().title("No. of Orders Served");
                        chart.data(data);
            ...
            heading = "The number of orders served by: " + selected;
            $("#dataTable").show();


            var s = $("#dataTable").DataTable({
                ajax: {
                    url: `/api/waiter-report/${id}/${start_date}/${end_date}`,
                    type: "GET"
                },
                dataSrc: 'data',
                columns: [
